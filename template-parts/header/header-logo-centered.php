...
<header class="header header-logo-centered w-full z-30 bg-quinary">
	...
	<div class="bg-quinary h-16 w-full z-30 lg:hidden">
		<button type="button" class="js-back absolute mt-5 left-4 header__back cursor-pointer"> 
			<span> <?php ground_icon( 'chevron-left', 'text-tertiary h-6 w-6' ); ?> </span> <?php _e( 'Indietro', 'ground' ); ?> 
		</button>
		...
		<div class="header__bar-mobile container py-2 bg-quinary grid grid-cols-12 items-center lg:flex lg:items-center lg:justify-between">
			...
			<div class="col-span-4 flex justify-end items-center">
				<button class="js-toggle js-cursor-hover mr-6 lg:mr-12" data-toggle-target=".search-form__wrapper html" data-toggle-class-name="is-search-open" aria-label="<?php esc_attr_e( 'Search', 'ground' ); ?>">
					<?php ground_icon( 'search', 'icon--filled text-tertiary h-6 w-6' ); ?>
				</button>
				...
				<div class="hidden lg:col-span-5 lg:relative lg:flex lg:justify-end lg:items-center">
					...
					<?php if ( class_exists( 'WooCommerce' ) ) : ?>
						<ul class=" relative z-0 lg:flex lg:items-center lg:space-x-5 lg:justify-end lg:m-0">
							<li class="text-lg lg:pl-3 lg:text-base"><a class="inline-block lg:py-auto" href="<?php echo get_permalink( wc_get_page_id( 'myaccount' ) ); ?>"><?php ground_icon( 'user', 'icon--filled text-tertiary h-6 w-6' ); ?></a></li>
							<li class="minicart-wrapper py-4 lg:inline-block basis-6"><?php get_template_part( 'template-parts/woocommerce/shopping-cart' ); ?> </li>
						</ul>
					<?php endif; ?>
