...
// ❌ WARNING: Large post__not_in lists slow the query.
'post__not_in' => $thousands_of_ids

// ✅ GOOD: Prefer narrowing the query itself (taxonomy/meta/date) so the
// exclusion list stays small. Only fall back to fetching a bounded set and
// filtering in PHP when the exclusion list is large AND you control pagination
// — array_filter after the query changes the result count and breaks paging.
```
