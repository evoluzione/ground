...
<div class="ground-block <?php echo $no_margin ? '' : 'my-12 lg:my-24'; ?> relative not-prose <?php echo esc_attr( ground_block_class( $block ) ); ?>">
	<div class="lg:grid lg:grid-cols-3 lg:gap-6">
		<div class="lg:col-span-2 lg:h-screen lg:sticky lg:top-0">
			<?php $image = $repeater[0]; ?>
				<?php if ( $image ) { ?>
					<figure class="overflow-hidden !rounded-none lg:!rounded-media h-full">
						<img class="w-full h-full object-cover !rounded-none lg:!rounded-media"
		...
		<div class="lg:space-y-6 carousel-css mt-6 lg:mt-0">
			<?php
			foreach ( $repeater as $key => $image ) :
				...
				?>
					...
					<?php if ( $image && $key >= 1 ) { ?>
						<div class="p-0 m-0">
							<figure class="overflow-hidden aspect-w-1 aspect-h-1 <?php echo $image_horizontal && ! $image_square ? 'lg:aspect-w-16 lg:aspect-h-9' : ''; ?> <?php echo ! $image_horizontal && ! $image_square ? 'lg:aspect-w-3 lg:aspect-h-4' : ''; ?> !rounded-media">
